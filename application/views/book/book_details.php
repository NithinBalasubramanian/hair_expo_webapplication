...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
  <h1 class="h3 mb-0 text-gray-800">Dashboard / Add Book Details</h1>
...
<div class="col-xl-12">
  <div class="card shadow mb-5">
    ...
    <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
      <h6 class="m-0 font-weight-bold text-primary">Book Details Form</h6>
    </div>
    <!-- Card Body -->
    <div class="card-body">
    <form class="form-horizontal" action="<?php echo base_url(''); ?>Add_invoice/invoice/invoice/add_invoice/print_invoice/<?php echo $edit_id; ?>" method="post" enctype="multipart/form-data" >
        <?php $book=$this->Admin_model->table_column('booking','id',$edit_id);
    if(count($book) > 0)
    {
        ...
        { ?>
              <div class="box-body">
               ...
               <div class="col-md-3">
                ...
                <label for="inputEmail3" class="col-sm-6 control-label">Contact:</label>
                  ...
                  <div class="col-sm-10">
                    <input type="number" name="contact" id="contact" class="form-control" max-length="10" value="<?php echo $row_book['contact']; ?>">
                  </div>
                </div>
                <div class="form-group col-md-6" id="already">
                <label for="inputEmail3" class="col-sm-6 control-label">Name : </label>
                <div class="col-sm-10">
                  <input class="form-control" type="hidden" name="customer_id" id="customer_id" value="<?php echo $row_book['customer_id']; ?>">

                  <?php $name=$this->Admin_model->table_column('customer','id',$row_book['customer_id']);
                  if(count($name) > 0)
                  {
                      foreach($name as $row_name)
                      { ?>
                       <input type="text" name="customer_name"  class="form-control" max-length="10" value="<?php echo strtoupper($row_name['customer_name']); ?>">
                ...
                <label for="inputEmail3" class="col-sm-6 control-label">Email (optional): </label>
                  ...
                  <div class="col-sm-10">
                    <input type="email" name="email" id="email" class="form-control"  value="<?php echo $row_book['email']; ?>" >
                  </div>
                </div>
                <div class="form-group col-md-6">
              ...
              <div class="row">
                <div class="form-group col-md-12 row">
                  ...
                  <?php
                         ...
                         $exp_data = explode(",", $row_book['service_id']);
                        $grand = 0;
                         foreach($exp_data as $row){
                         $service=$this->Admin_model->table_column('service','id',$row);
                            foreach($service as $row)
                           {
                             $grand+=$row['cost']; ?>
                           <div class="col-md-12 row">
                           <input type="hidden" name="service[]" value="<?php echo $row['id']; ?>">
                               <input type="text" name="service_pre" class="col-md-6 form-control" value="<?php  echo $row['service_type']; ?>" style="
                                  margin: 15px 35px;"><p style=" margin: 15px -15px;"> Rs:</p> 
                              <input type="text" name="service_pre" class="col-md-4 form-control" readonly="true" value="<?php  echo $row['cost']; ?>" style="
                                  margin: 15px 35px;">
                              <!--<p class="col-md-6"><?php  echo $row['service_type']; ?></p><p class="col-md-6">- Rs: <?php  echo $row['cost']; ?></p>-->
                             </div>
                        <?php   }
                         }
              ...
              <div class="row">
                ...
                <div class="col-md-8">
                    ...
                    <span class="col-md-4  gst_disp side_list">Grand Total :<?php echo $grand; ?></span>
                    <span class="col-md-4 grand_net  side_list"></span>
                </div>
              </div>
              <input type="hidden" id="grand" value="<?php echo $grand; ?>">
